    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                @endif
                <div class="card-body">
                    <form method="POST" action="{{ route('sevkham.store') }}">
                        @csrf
                        ...
                        <div class="form-group row" name="boyahane">
                            <label for="boyahane_id" class="col-md-4 col-form-label text-md-right">{{ __('Boyahane Talimatı') }}</label>

                            <div class="col-md-6">
                                <select name='boyahane_id' id="boyahane_id" class="form-control  @error('boyahane_id') is-invalid @enderror" >
                                    <option value="">Seçiniz..</option>
                                    @foreach ($boyahane as $list)
                                    <option value="{{$list->id}}">{{$list->no.' '.$list->firma->name}}</option>
                                    @endforeach
                                </select>
                                @error('boyahane_id')
                                <span class="invalid-feedback" role="alert">
...
<script type="text/javascript">
    ...
    if(sid){
            $.ajax({
             type:"get",
             url:'{{url('/sevkham/firmadetay')}}/'+sid, 
             ...
             {     var kayitSay = res.length;  
                if(kayitSay > 0)
                    {//console.log; 
                        ...
                        $("div[name='boyahane']").hide();
              
               $('#firmatipi_id').change(function(){
                    if (($('#firmatipi_id option:selected').text()) =='KUMAŞ BOYAHANE')
                        {
                        $("div[name='boyahane']").show();
                        }
                        else 
                        $("div[name='boyahane']").hide();
                });
