...
<script type="text/javascript">
    $('input[name=iplikmarka]').change(function(){
        var a= $(this).attr('name');
        var val= $(this).val();
        id = $(this).attr('id').substring(a.length);
        if (id==1){
                $('input[name*=iplikmarka]').val(val);
            }
    })
     $('input[name=partino]').change(function(){
        var a= $(this).attr('name');
        var val= $(this).val();
        id= $(this).attr('id').substring(a.length);
        if(id==1){
                $('input[name*=partino]').val(val);
            }
    })
     $('select[name=iplikcins_id]').change(function(){
        var a= $(this).attr('name');
        var val= $(this).val();
        id= $(this).attr('id').substring(a.length);
       // var b= $('#iplikcins_id'+id+' option:selected').text();
        var c= $('#iplikcins_id'+id+' option:selected').val();
        if (id==1){
                //$('select[id*=iplikcins_id]').append('<option value="'+a+'">'+b+'</option>');
                $("select[name*=iplikcins_id] option[value="+c+"]").attr('selected', 'selected');
            }
    })
      $('select[name=boyacins_id]').change(function(){
        var a= $(this).attr('name');
        var val= $(this).val();
        id= $(this).attr('id').substring(a.length);
        var b= $('#boyacins_id'+id+' option:selected').text();
        if (id==1){
                //$('select[name*=boyacins_id]').empty();
                $("select[name*=boyacins_id] option:contains("+b+")").attr('selected', 'selected');
                if(b=='HAM'){
                $("input[name*=renk]").val('HAM');
                $("input[name*=rno]").val('HAM');
                }
                else {
                $("input[name*=renk]").val('');
                $("input[name*=rno]").val('');
      ...
      $('input[name=iplikno]').change(function(){
        var a= $(this).attr('name');
        var val= $(this).val();
        id= $(this).attr('id').substring(a.length);
        if(id==1){
                $('input[name*=iplikno]').val(val);
            }
    })
      $('input[name=ne]').change(function(){
        var a= $(this).attr('name');
        var val= $(this).val();
        id= $(this).attr('id').substring(a.length);
        if(id==1){
                $('input[name*=ne]').val(val);
            }
    })
      $('input[name=renk]').change(function(){
        var a= $(this).attr('name');
        var val= $(this).val();
        id= $(this).attr('id').substring(a.length);
        if(id==1){
                $('input[name*=renk]').val(val);
            }
    })
      $('input[name=rno]').change(function(){
        var a= $(this).attr('name');
        var val= $(this).val();
        id= $(this).attr('id').substring(a.length);
        if(id==1){
                $('input[name*=rno]').val(val);
            }
    })
       $('input[name=sim]').change(function(){
        ...
        id= $(this).attr('id').substring(a.length);
        if(id==1){
                $('input[name*=sim]').val(val);
            }
    })
      $('input[name=sno]').change(function(){
        var a= $(this).attr('name');
        var val= $(this).val();
        id= $(this).attr('id').substring(a.length);
        if(id==1){
                $('input[name*=sno]').val(val);
            }
    })
      /*$('input[name=brutmktr]').change(function(){
        var a= $(this).attr('name');
        var val= $(this).val();
        id= $(this).attr('id').substring(a.length);
        if(id==1){
                $('input[name*=brutmktr]').val(val);
            }
    })*/
       $('select[name=unit_id]').change(function(){
        ...
        var val= $(this).val();
        id= $(this).attr('id').substring(a.length);
        var b= $('#unit_id'+id+' option:selected').text();
        if (id==1){
                //$('select[id*=unit_id]').append('<option value="'+a+'">'+b+'</option>');
                $("select[name*=unit_id] option:contains("+b+")").attr('selected', 'selected');
            }
    })
     $('input[id*=miktar]').change(function(){
